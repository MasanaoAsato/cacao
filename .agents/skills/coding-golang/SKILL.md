---
name: coding-golang
description: When a user requests a specific coding skill, the agent refers to that skill to carry out the implementation.
---

# coding golang
This repository contains an implementation in Go.
You must implement tests.
Must do `mise run lint` to check for linting errors.
Must do `mise run fmt` to check for formatting errors.
Must check doing `mise run test` to check for test errors.
There are some good skills for Go that can be used to improve the quality of the code, such as:

-golang-code-style
-golang-error-handling
-golang-lint
-golang-naming
-golang-observability
-golang-performance
-golang-popular-libraries
-golang-security
-golang-testing
-golang-troubleshooting

After implementation, you must call the "code-review" skill from a separate agent to receive feedback on the review.
