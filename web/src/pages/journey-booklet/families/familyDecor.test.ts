...
const familyDesign: FamilyDecorDesign = {
...
/** Every page of a built document, as a scene hands its own pages over. */
function pagesOf(root: HTMLElement): HTMLElement[] {
	return Array.from(root.querySelectorAll<HTMLElement>("[data-booklet-page]"));
}
...
describe("実ページからの装飾計測", () => {
	...
	it("正常系: 描画済みの装飾と解決した配置が一致すればページを返す", () => {
		const pages = prepareFamilyDecorPages(
			pagesOf(buildPage()),
			familyDesign,
	...
	it("境界値: CSSピクセル変換による0.05mm以内の丸め差を許容する", () => {
		const pages = prepareFamilyDecorPages(
			pagesOf(buildPage({ drawnBounds: "10.04,9.96,10.03,9.97" })),
			familyDesign,
			new Map([["page-1", [COMPASS]]]),
		);
	...
	it("異常系: 描画された境界が配置と違えば準備完了にしない", () => {
		expect(() =>
			prepareFamilyDecorPages(
				pagesOf(buildPage({ drawnBounds: "10.00,10.00,12.00,12.00" })),
				familyDesign,
				new Map([["page-1", [COMPASS]]]),
			),
		).toThrow(BookletLayoutError);
		expect(() =>
			prepareFamilyDecorPages(pagesOf(buildPage()), familyDesign, new Map()),
		).toThrow(BookletLayoutError);
	});
	...
	it("異常系: 未知のanchor種類は定義エラーとして扱う", () => {
		expect(() =>
			prepareFamilyDecorPages(
				pagesOf(buildPage({ anchorKind: "photo" })),
				familyDesign,
				new Map([["page-1", [COMPASS]]]),
			),
