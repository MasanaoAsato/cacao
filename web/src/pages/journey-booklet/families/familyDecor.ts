...
/**
 * Reads each page's anchors and text rects, resolves the family's decor
 * against them, and confirms the drawn layers match. Placement failures are
 * raised so the scene turns to `error` instead of printing a page whose decor
 * was silently dropped (20.6). Only the scene's own pages are checked.
 */
export function prepareFamilyDecorPages(
	pageElements: readonly HTMLElement[],
	design: Pick<FamilyDecorDesign, "decorAssetIds" | "familyId" | "seedToken">,
