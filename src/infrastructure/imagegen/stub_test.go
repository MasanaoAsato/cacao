...
func TestStubRejectsInvalidBriefAndInjectedError(t *testing.T) {
	...
	t.Run("injected error", func(t *testing.T) {
		...
		stub := NewStub(WithStubError(wantErr))
...
func TestStubRejectsCanceledContext(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	cancel()

	_, err := NewStub().Generate(ctx, newTestImageBrief(t))
