...
// Stub はGPUや外部APIを使わず固定PNGを返す画像生成器である。
type Stub struct {
...
// StubOption は Stub の構成を変更する。
type StubOption func(*Stub)

// WithStubError は指定したエラーを生成時に返す設定を作る。
func WithStubError(err error) StubOption {
	return func(stub *Stub) {
		stub.ErrOn = err
	}
}

// NewStub は固定PNGを返すStubを生成する。
func NewStub(options ...StubOption) *Stub {
	stub := &Stub{content: newStubPNG()}
...
var _ domainservice.ImageGenerator = (*Stub)(nil)

// Generate は有効な ImageBrief に対して固定PNGを返す。
func (s *Stub) Generate(
	ctx context.Context,
