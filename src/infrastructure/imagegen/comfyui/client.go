...
// defaultLimits は WithLimits が指定されないときの画像上限。
var defaultLimits = imagecontent.Limits{
...
// WithLimits は /view で受け入れる画像の上限（byte 数・寸法・画素数）を設定する。
func WithLimits(limits imagecontent.Limits) ClientOption {
	return func(client *Client) {
...
func (c *Client) View(ctx context.Context, output OutputReference) (Image, error) {
	...
	body, readErr := readResponseBody(response, c.limits.MaxBytes)
	if response.StatusCode >= http.StatusInternalServerError {
		if readErr != nil {
	...
	info, err := imagecontent.Inspect(body, c.limits)
	if err != nil {
