...
// NewGenerator はComfyUI generatorを作成する。
func NewGenerator(
	baseURL string,
	workflowPath string,
	manifestPath string,
	...
	workflow, err := NewWorkflow(workflowPath, manifestPath)
	if err != nil {
...
// NewGeneratorFromBytes はJSON bytesからComfyUI generatorを作成する。
func NewGeneratorFromBytes(
	baseURL string,
	workflowData []byte,
	manifestData []byte,
	options ...ClientOption,
	...
	workflow, err := NewWorkflowFromBytes(workflowData, manifestData)
	if err != nil {
		return nil, fmt.Errorf("load comfyui workflow: %w", err)
	}
