...
func TestFileSystemImageStorageRejectsConcurrentDifferentContentAcrossInstances(t *testing.T) {
	...
	root := t.TempDir()
	firstStorage := newTestFileSystemImageStorageAt(t, root, ImageStorageConfig{})
	secondStorage := newTestFileSystemImageStorageAt(t, root, ImageStorageConfig{})
	imageID := value_object.NewID()
	first := newTestGeneratedImage(
		t,
		"image/png",
	...
	for _, input := range []struct {
		storage *FileSystemImageStorage
		...
		waitGroup.Add(1)
		go func(storage *FileSystemImageStorage, generatedImage domainservice.GeneratedImage) {
...
func newTestFileSystemImageStorage(
	...
	t.Helper()
	return newTestFileSystemImageStorageAt(t, t.TempDir(), config)
}
...
func newTestFileSystemImageStorageAt(
	...
	if config.MaxPixels == 0 {
		config.MaxPixels = 4096 * 4096
	}

	storage, err := NewFileSystemImageStorage(config)
	if err != nil {
		t.Fatalf("NewFileSystemImageStorage() error = %v", err)
	}
	t.Cleanup(func() {
