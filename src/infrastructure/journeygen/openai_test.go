...
func TestOpenAIGenerator_Generate(t *testing.T) {
	...
	generator := NewOpenAIGenerator(client, "gpt-4o-mini", false)
...
func TestOpenAIGenerator_Generate_InvalidLLMOutput(t *testing.T) {
	client := newOpenAITestClient(t, func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
...
// TestOpenAIGenerator_Generate_UsesJSONSchema はリクエストボディの text.format が
...
func TestOpenAIGenerator_Generate_UsesJSONSchema(t *testing.T) {
	llmOutput := `{"days":[{"date":"2026-08-01","spots":[{"name":"浅草寺","description":"東京最古の寺院","startAt":"2026-08-01T09:00:00+09:00","estimatedCost":{"amount":0,"currency":"JPY"}}],"legs":[{"from":"東京（出発地）","mode":"walk","durationMinutes":1,"cost":{"amount":0,"currency":"JPY"}}]}]}`
	...
	generator := NewOpenAIGenerator(client, "gpt-4o-mini", true)
	start := time.Date(2026, 8, 1, 0, 0, 0, 0, time.UTC)
	end := time.Date(2026, 8, 3, 0, 0, 0, 0, time.UTC)
	req := newStubTestJourneyRequest(t, start, end, 30000)
...
// TestOpenAIGenerator_Generate_WithWebSearch は Web Search 有効時に
// リクエストボディの tools に web_search が含まれることを検証する。
// これは「Web Search が有効化された状態で OpenAI API が呼ばれているか」を保証する境界値に近い正常系テスト。
func TestOpenAIGenerator_Generate_WithWebSearch(t *testing.T) {
	llmOutput := `{"days":[{"date":"2026-08-01","spots":[{"name":"浅草寺","description":"東京最古の寺院","startAt":"2026-08-01T09:00:00+09:00","estimatedCost":{"amount":0,"currency":"JPY"}}],"legs":[{"from":"東京（出発地）","mode":"walk","durationMinutes":1,"cost":{"amount":0,"currency":"JPY"}}]}]}`
...
// TestOpenAIGenerator_Generate_WithoutWebSearch は Web Search 無効時に
// リクエストボディの tools に web_search が含まれないことを検証する。
// 未設定時は Web Search が無効であることを保証する境界値テスト。
func TestOpenAIGenerator_Generate_WithoutWebSearch(t *testing.T) {
