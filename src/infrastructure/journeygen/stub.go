...
// Stub は JourneyGenerator ドメインサービスのスタブ実装。
...
// NewStub は Stub を生成する。
func NewStub() *Stub {
	return &Stub{}
}

// Generate は JourneyRequest の条件から固定のダミー旅程を生成する。
func (g *Stub) Generate(_ context.Context, request entity.JourneyRequest) (service.GeneratedRoute, error) {
	if g.ErrOn != nil {
